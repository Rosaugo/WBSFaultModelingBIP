...
The reminder of this seciton is organized as follows. First, we introduce serveral categories of standard requirements of WBS in quantitative manner. Then we discuss our choice of probability distribution and estimation methods as well as the experimental results for each requirement.
...
\begin{table}[htbp]
	...
	\begin{center}
		...
		\begin{tabular}{|c|c|@{}p{0.002\linewidth}<{\centering}@{}|c|}
			...
			\hline
			{\tabincell{c}{Pumps}}&{\tabincell{c}{Green Hydraulic Pump\\ Blue Hydraulic Pump\\ Accumulator}}&&{$3.3\times10^{-5}$}\\
			\hline
			\multirow{2}*{\tabincell{c}{Valves}}&{\tabincell{c}{Shutoff Valve\\Anti-Skid Valve\\Green Meter Valve\\ Blue Meter Valve}}&&{$1.3\times10^{-5}$}\\
			\cline{2-4}
			&{Selector Valve}&&{$1.3\times10^{-5}$}\\
			\hline
...
Following the methods above, we generate a comparison of all the four architectures on the results of PE as shown in table 5. The overall results are based on the following evaluation criteria: 
